...
var getStatussUrl = "/java_s04/api/v1.1/posts";
	...
	if (data.length === 0) {
		...
		$.each(data, function(index, employee) {
			...
			row.append($('<td>').append(
					$('<button>').text("編集").attr("type", "button").attr(
							"onclick", "findById(" + employee.id + ')')));
			row.append($('<td>').append(
					$('<button>').text("削除").attr("type", "button").attr(
...
// Status検索のために作ろうと思ったけどめんどくさそうなのでやめます。以下。
function makeStatusSelection(selectionId, employee) {
	...
	$.ajax({
		...
		success : function(data, textStatus, jqXHR) {
			$.each(data, function(index, status) {
				var newOption = $('<option>').val(status.id).text(status.name);
